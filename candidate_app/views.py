...
def process_excel_file(full_file_path):
    ...
    #     empexceldata = pd.DataFrame(odsdata)
        ...
        HttpResponse('Select correct format  Acceptable formats -- .xlsx,.xlx,.xltx,.xlt,.xlsm')
        ...
        try:
            if empexceldata is not None:
                ...
                for index, row in empexceldata.iterrows():
                    ...
                    if any(row[['Email', 'Candidate Name',]].isnull()):
                        continue 
                    ...
                    due_date = row.loc['Due Date'] if 'Due Date' in row else (row.loc['due date'] if 'due date' in row else (row.loc['Due'] if 'Due' in row else row.loc['DUE DATE']))
                    candidate_name = row.loc['Candidate Name'] if 'Candidate Name' in row else (row.loc['candidate name'] if 'candidate name' in row else (row.loc['Candidate name'] if 'Candidate name' in row else row.loc['CANDIDATE NAME']))
                    highest_qualification = row.loc['Highest Qualification'] if 'Highest Qualification' in row else (row.loc['Highest qualification'] if 'Highest qualification' in row else (row.loc['highest qualification'] if 'highest qualification' in row else row.loc['HIGHEST QUALIFICATION']))
                    ...
                    filtered_skill_names = [skill_name.strip() for skill_name in skill_names if len(skill_name.strip()) <= 50]

                    skills = [Skill.objects.get_or_create(name=skill_name, user=request.user)[0] for skill_name in filtered_skill_names]
...
########## select and delete candidate
@recruiter_login_required
def selected_delete(request):
    ...
    def post(self, request):
        if request.method == "POST":
            ...
            candidate = Candidate(
                ...
                resume=resume,
                status=status,
                additional_status=rejection_reason,
                user=request.user
            )
            candidate.save()
...
class Updatecandidate(LoginRequiredMixin,UpdateView):
    ...
    def post(self, request, pk):
        ...
        if candidate.relevent_experience == "":
            candidate.relevent_experience=None
...
def saved_candidates(request):
    if request.user.is_authenticated:
        count = Candidate.objects.filter(user=request.user, status='On Boarded').count()
        return JsonResponse({'count': count})
    else:
        return JsonResponse({'count': 0})
